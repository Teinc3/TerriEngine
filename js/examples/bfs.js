...
function cycleLoop(currentCycle, prevIFSes) { // Change prevIFSes to prevSimState in the future
    ...
    while (engine.tick < (currentCycle - 1) * 100) {
        engine.update();
    }
    ...
    combinLoop: for (let combinValue = 0; combinValue < 2 ** (numIFSes + 1); combinValue++) { // 000... to 111...
        ...
        piaiLoop: for (let piai = 0; piai < ((baseCycleIFSes.length == 0 || baseCycleIFSes[0].remarks !== "PIAI") ? 1 : 2); piai++) {
            ...
            if (cycleIFSes.length !== 0) {
                ...
                let auInterval = accumulatedLand > 1E4 ? 2 : accumulatedLand > 1E3 ? 3 : 4;
                ...
                while (index < cycleIFSes.length) {
                    ...
                    while (closestAU < IFS.IFS) { // At cAU = IFS[i], the auDiff should be counted towards i and not i-1, so we use < instead of <=
                        ...
                        auDiffs++;
                        if (config.options?.checkAllAUInterval || auInterval == 4) { // This helps us check if we need to change the auInterval (if we change it, we don't have to check again anymore)
                            accumulatedLand += 4 * (layerFormula(accumulatedLand) + 1);
                            if (accumulatedLand > 1E4) auInterval = 2;
                            else if (accumulatedLand > 1E3) auInterval = 3;
                        }
                    }
                    IFS.CAUT = closestAU;
            ...
            let engineResult;
            while (engine.tick < currentCycle * 100) {
...
function pruneResults(results, cycle) {
    ...
    for (const land in landMap) {
        ...
        for (const result of landMap[land]) {
            const resultTroops = aggregateTroops(result),
                maxTroops = aggregateTroops(maxTroopsResult);
            if (resultTroops > maxTroops || resultTroops == maxTroops && result.IFSes.length < maxTroopsResult.IFSes.length) { // Easier to do this opening
    ...
    for (let i = 0; i < selected.length; i++) {
        for (let j = 0; j < selected.length; j++) {
            if (i !== j && selected[j].land > selected[i].land && aggregateTroops(selected[j]) >= aggregateTroops(selected[i])) {
                selected.splice(i, 1);
    ...
    if (config.options?.pruneMoreTroops) {
        for (let i = 0; i < selected.length; i++) {
            ...
            const nextCycleStart = (cycle + 1) * 100;
            ...
            for (let j = 0; j < selected.length; j++) {
                if (i !== j && selected[j].land > selected[i].land) {
                    ...
                    const diffBefore = selected[i].troops - selected[j].troops; // No aggregate, cuz remaining troops return in future
    ...
    function aggregateTroops(result) {
        return result.troops + result.remaining;
    }
}
